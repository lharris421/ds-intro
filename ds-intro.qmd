...
nasem_tbl |>
  gt() |>
  tab_header(title = md("**NASEM Taxonomy of Computational Reliability**")) |>
  cols_label(
...
Quarto starts from a fresh session every time it is rendered, not relying on objects in your environment.

It also runs linearly from top to bottom, forcing code to execute sequentially so every object is created before it is used, eliminating hidden dependencies and making the workflow fully reproducible.
...
::: {.callout-tip}
`filter()` does not alter the original tibble. If you want to save the filtered data, you must assign the result to a new object. The same is true of other `dplyr` and `tidyr` functions.
:::
...
```{r}
penguins |> 
  filter(species == "Adelie") |> slice_head(n = 10) |> gt()
```
...
Here we filter down to Adelie penguins from Torgersen island:
...
Now lets say we want 3 different colored smooth lines for the 3 origin airports,
